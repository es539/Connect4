...
object Connect4Controller extends Controller {
  override def movementValidation(gameBoard: Array[Array[Piece]], state: State): MoveValidation = {
    var i = 0
    while (i < 6) {
      if (gameBoard(i)(state.currentCol) == null) {
        gameBoard(i)(state.currentCol) = if (state.color == 1)
          new Piece(i, state.currentCol, Connect4En.Red)
        else
          new Piece(i, state.currentCol, Connect4En.Yellow)
        return new MoveValidation(new State(i, state.currentCol, 0), true)
      } else
        i += 1
    }
  ...
  override def checkEndGame(board: Array[Array[Piece]], turn: Int): Boolean = {
   ...
   * @param j         the parameter for the columns that we want to check
  ...
  private def checkDirection(gameBoard: Array[Array[Piece]], row: Int, col: Int, i: Int, j: Int): Boolean = {
    val turn = gameBoard(row)(col).color
    for (move <- 1 to 3) {
      val newRow = row + move * i
      val newCol = col + move * j
      if (newRow < 0 || newRow > 5 || newCol < 0 || newCol > 6
        || gameBoard(newRow)(newCol) == null || gameBoard(newRow)(newCol).color != turn) return false
    }
    true
  }

  override def getScore(gameBoard: Array[Array[Piece]], turn: Int): Array[Int] = {
    var score: Array[Int] = Array(0, 0)
    return score
  }

  def calculateScore(gameBoard: Array[Array[Piece]], turn: Int): Boolean = {
    val turns: Array[Int] = Array(Connect4En.Yellow, Connect4En.Red)
    val dx: Array[Int] = Array(1, 1, 0, 0, -1, -1, 1, -1)
    val dy: Array[Int] = Array(1, -1, 1, -1, -1, 1, 0, 0)

    gameBoard.foreach(_.foreach(piece => {
      if (piece != null && piece.color == turns(turn)) {
        for (i <- 0 until 8) {
