...
class Connect4AI extends Player {
  private val turns: Array[Int] = Array(Connect4En.Yellow, Connect4En.Red)
  ...
  override def Movement(source: Node): Unit = {
    var move: State = null
    if (miniMaxType == "Minimax")
      move = miniMax(gameBoard, color, depth).getKey
    else
      move = miniMaxAlphaBeta(gameBoard, color, -1000, 1000, depth).getKey

    gameBoard(move.currentRow)(move.currentCol) = new Piece(move.currentRow, move.currentCol, color)

    var src: Node = null
    Connect4Drawer.gameBoard.getChildren.forEach(node => {
      if (GridPane.getColumnIndex(node) == move.currentCol && GridPane.getRowIndex(node) == move.currentRow) {
        src = node
   ...
   */
  private def miniMax(board: Array[Array[Piece]], turn: Int, depth: Int): Pair[State, Int] = {
    if (Connect4Controller.checkEndGame(board))
    ...
    for (col <- 0 until Connect4En.ColLen) {
      ...
      if (freeRow < Connect4En.RowLen) {
        board(freeRow)(col) = new Piece(freeRow, col, turn)

        val curScore: Pair[State, Int] = miniMax(board, 1 - turn, depth - 1)
        board(freeRow)(col) = null

        if (turn == 0) {
          if (curScore.getValue < score) {
            score = curScore.getValue
            bestMove = new State(freeRow, col, turn)
          }
        } else {
          if (curScore.getValue > score) {
            score = curScore.getValue
  ...
  private def miniMaxAlphaBeta(board: Array[Array[Piece]], turn: Int, a: Int, b: Int, depth: Int): Pair[State, Int] = {
    if (Connect4Controller.checkEndGame(board))
      return new Pair[State, Int](null, 0)
    if (depth == 0)
      return new Pair[State, Int](null, if (turn == 0) estimate(gameBoard, turn) * -1 else estimate(gameBoard, turn))
    ...
    for (col <- 0 until Connect4En.ColLen) {
      ...
      if (freeRow < Connect4En.RowLen) {
        ...
        val curScore: Pair[State, Int] = miniMaxAlphaBeta(board, 1 - turn, alpha, beta, depth - 1)
        board(freeRow)(col) = null
