  ...
  override def movementDraw(source: Node, state: State, arg: Node): Unit = {
    breakable {
      gameBoard.getChildren.forEach {
        case node@(x: Circle) if GridPane.getColumnIndex(node) == state.currentCol
          && GridPane.getRowIndex(node) == 5 - state.currentRow =>
          if (state.color == 1) x.setFill(Color.rgb(164, 43, 57))
